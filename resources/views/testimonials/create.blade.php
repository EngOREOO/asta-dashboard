...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-6">
  <div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          ...
          <div>
            <h1 class="font-bold text-gray-800" style="font-size: 2rem;">إضافة شهادة جديدة</h1>
            <p class="text-gray-600" style="font-size: 1.1rem;">أضف شهادة جديدة من العميل</p>
          </div>
        </div>
    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20">
      ...
      <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
        @csrf
        ...
        <div class="mb-8">
          ...
          <div class="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-blue-500 transition-colors duration-300">
            ...
            <label for="user_image" 
              ...
              <i class="ti ti-upload mr-2"></i>اختيار صورة
            </label>
          ...
          <div class="flex items-center space-x-2 rtl:space-x-reverse">
            @for($i = 1; $i <= 5; $i++)
              <label class="cursor-pointer">
                ...
                <i class="ti ti-star text-3xl rating-star {{ old('rating', 5) >= $i ? 'text-yellow-400' : 'text-gray-300' }}" 
                   data-rating="{{ $i }}"></i>
              </label>
            @endfor
            <span class="mr-4 text-gray-600 font-semibold" id="rating-text">{{ old('rating', 5) }}/5</span>
          </div>
          @error('rating')
            <p class="text-red-600 mt-2 text-sm">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-8">
          ...
          <textarea name="comment" rows="6" 
                    ...
                    placeholder="أدخل تعليق العميل" required>{{ old('comment') }}</textarea>
          @error('comment')
            <p class="text-red-600 mt-2 text-sm">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="flex items-center justify-end space-x-4 rtl:space-x-reverse">
          ...
          <button type="submit" 
                  class="bg-gradient-to-r from-green-600 to-teal-600 text-white px-8 py-4 rounded-2xl font-semibold hover:from-green-700 hover:to-teal-700 transition-all duration-300 shadow-lg hover:shadow-xl">
            <i class="ti ti-check mr-2"></i>حفظ الشهادة
          </button>
        </div>
      </form>
    </div>
