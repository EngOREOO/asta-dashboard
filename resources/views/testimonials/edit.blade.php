    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20">
      ...
      <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="p-8">
        ...
        <div class="mb-8">
          ...
          <input type="text" name="user_name" value="{{ old('user_name', $testimonial->user_name) }}" 
        ...
        <div class="mb-8">
          ...
          <div class="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-blue-500 transition-colors duration-300">
            ...
            <label for="user_image" 
              ...
              <i class="ti ti-upload mr-2"></i>{{ $testimonial->user_image ? 'تغيير الصورة' : 'اختيار صورة' }}
            </label>
          ...
          <div class="flex items-center space-x-2 rtl:space-x-reverse">
            @for($i = 1; $i <= 5; $i++)
              <label class="cursor-pointer">
                ...
                <i class="ti ti-star text-3xl rating-star {{ old('rating', $testimonial->rating) >= $i ? 'text-yellow-400' : 'text-gray-300' }}" 
                   data-rating="{{ $i }}"></i>
              </label>
            @endfor
            <span class="mr-4 text-gray-600 font-semibold" id="rating-text">{{ old('rating', $testimonial->rating) }}/5</span>
          </div>
          @error('rating')
            <p class="text-red-600 mt-2 text-sm">{{ $message }}</p>
          @enderror
        ...
        <div class="mb-8">
          ...
          <textarea name="comment" rows="6" 
                    ...
                    placeholder="أدخل تعليق العميل" required>{{ old('comment', $testimonial->comment) }}</textarea>
          @error('comment')
            <p class="text-red-600 mt-2 text-sm">{{ $message }}</p>
          @enderror
        </div>
