  ...
  <div class="space-y-8 p-6">
    ...
    <div class="bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-600 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
      <div class="relative z-10 flex items-center justify-between">
        ...
        <a href="{{ route('users.index') }}" class="inline-flex items-center px-6 py-3 bg-white/10 hover:bg-white/20 rounded-xl backdrop-blur-sm transition-all duration-300">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div class="xl:col-span-2 bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-8">
        ...
        @if(method_exists($user,'instructorCourses') ? $user->instructorCourses->count() > 0 : $user->courses->count() > 0)
          ...
          <table class="w-full" id="user-courses-table">
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              <tr class="hover:bg-gray-50 transition-colors duration-200">
                <td class="px-4 py-3 text-center text-gray-600">{{ $course->id }}</td>
                ...
                <td class="px-4 py-3">
                  <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    {{ method_exists($user,'instructorCourses') ? 'محاضر' : 'طالب' }}
                  </span>
                </td>
                <td class="px-4 py-3">
                  <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $course->status === 'approved' ? 'bg-green-100 text-green-800' : ($course->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-700') }}">
                    {{ $statusAr }}
                  </span>
                </td>
    ...
    <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-8">
      ...
      <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-6 mb-6 border border-indigo-200">
        ...
        <form method="POST" action="{{ route('users.roles.assign', $user) }}" class="space-y-4">
          @csrf
            ...
            <div>
              ...
              <select name="role_to_add" class="w-full border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-300">
                ...
                @foreach($allRoles as $r)
                  @if(!$user->roles->pluck('name')->contains($r->name))
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-200">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <span class="font-bold text-gray-800 text-lg">{{ $role->name }}</span>
                  <p class="text-gray-500 text-sm">{{ $role->permissions->count() }} صلاحية</p>
                </div>
              </div>
              <form method="POST" action="{{ route('users.roles.revoke', [$user, $role]) }}" onsubmit="return confirm('هل أنت متأكد من إزالة هذا الدور؟');">
                @csrf @method('DELETE')
        ...
        <div class="bg-white/50 rounded-2xl p-6 border border-gray-200">
          ...
          <div class="mb-4 p-4 bg-emerald-50 rounded-xl border border-emerald-200">
            ...
            <form method="POST" action="{{ route('users.permissions.give', $user) }}" class="space-y-3">
              @csrf
              <div>
                ...
                <select name="permissions[]" multiple class="w-full border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all duration-300">
                  @foreach($permissionGroups as $groupName => $permissions)
                    <optgroup label="{{ $groupName }}">
                      @foreach($permissions as $permKey => $permAr)
                        @if(!in_array($permKey, $userDirectPerms))
                          <option value="{{ $permKey }}">{{ $permAr }}</option>
                        @endif
                      @endforeach
                    </optgroup>
                  @endforeach
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between mb-3">
              ...
              <span class="text-sm text-gray-500">{{ $user->getDirectPermissions()->count() }} صلاحية</span>
            </div>
            <div class="max-h-48 overflow-y-auto space-y-2">
              ...
              <div class="flex items-center justify-between p-3 bg-white rounded-xl border border-gray-200">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-gray-800 text-sm">{{ $permissionLabels[$perm->name] ?? $perm->name }}</span>
                </div>
                <form method="POST" action="{{ route('users.permissions.revoke', [$user, $perm]) }}" onsubmit="return confirm('هل أنت متأكد من إزالة هذه الصلاحية؟');">
                  @csrf @method('DELETE')
