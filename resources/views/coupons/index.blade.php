    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 animate-slide-up">
      <div class="bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-600 px-8 py-6 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center justify-between">
          ...
          <a href="{{ route('coupons.create') }}" class="group inline-flex items-center px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-2xl shadow hover:shadow-xl hover:from-emerald-700 hover:to-teal-700 transition" style="font-size: 1.3rem;">
    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6">
      ...
      <form method="GET" class="mb-6 grid grid-cols-1 md:grid-cols-6 gap-4">
        <div>
          ...
          <input type="text" name="code" value="{{ $filters['code'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
        </div>
        <div>
          <label class="block text-sm text-gray-600 mb-1">الخصم من (%)</label>
          <input type="number" step="0.01" name="percentage_min" value="{{ $filters['percentage_min'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
        </div>
        <div>
          <label class="block text-sm text-gray-600 mb-1">الخصم إلى (%)</label>
          <input type="number" step="0.01" name="percentage_max" value="{{ $filters['percentage_max'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
        </div>
        <div>
          ...
          <select name="applies_to" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;">
            ...
            <option value="all" @selected(($filters['applies_to'] ?? '')==='all')>كل الدورات</option>
            <option value="selected" @selected(($filters['applies_to'] ?? '')==='selected')>دورات محددة</option>
          </select>
        </div>
        <div>
          ...
          <select name="status" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;">
            ...
            <option value="inactive" @selected(($filters['status'] ?? '')==='inactive')>متوقف</option>
          </select>
        </div>
        <div class="md:col-span-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input type="date" name="starts_from" value="{{ $filters['starts_from'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">إلى تاريخ (بداية)</label>
            <input type="date" name="starts_to" value="{{ $filters['starts_to'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">من تاريخ (نهاية)</label>
            <input type="date" name="ends_from" value="{{ $filters['ends_from'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">إلى تاريخ (نهاية)</label>
            <input type="date" name="ends_to" value="{{ $filters['ends_to'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" style="font-size: 1.15rem;" />
          </div>
        </div>
        <div class="md:col-span-6 flex items-end gap-3">
          ...
          <a href="{{ route('coupons.index') }}" class="coupon-action-btn coupon-action-btn--secondary px-5 py-2.5 rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200">مسح</a>
        </div>
      </form>
          ...
          <table class="admin-table coupon-table-lg" id="coupons-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">{{ $coupon->percentage }}%</td>
                <td class="text-center">{{ $coupon->applies_to === 'all' ? 'كل الدورات' : 'دورات محددة' }}</td>
                <td class="text-center">{{ $coupon->applies_to === 'all' ? '—' : $coupon->courses_count }}</td>
                <td class="text-center">0</td>
                <td class="text-center">{{ optional($coupon->starts_at)->format('Y-n-j') }} - {{ optional($coupon->ends_at)->format('Y-n-j') }}</td>
                <td class="text-center">
                  <span class="inline-flex px-2.5 py-0.5 rounded-full font-medium {{ $coupon->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">{{ $coupon->is_active ? 'نشط' : 'متوقف' }}</span>
                </td>
                <td class="text-center">
                  <div class="inline-flex items-center gap-2">
                    <a href="{{ route('coupons.show', $coupon) }}" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-blue-700 bg-blue-100 hover:bg-blue-200" title="عرض"><i class="ti ti-eye text-base"></i></a>
                    <a href="{{ route('coupons.edit', $coupon) }}" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-gray-700 bg-gray-100 hover:bg-gray-200" title="تعديل"><i class="ti ti-edit text-base"></i></a>
                    <form action="{{ route('coupons.destroy', $coupon) }}" method="POST" class="inline" onsubmit="return confirm('حذف الكوبون؟')">
                      @csrf
                      @method('DELETE')
