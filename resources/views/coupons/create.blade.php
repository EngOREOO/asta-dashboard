    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 animate-slide-up">
      <div class="bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-600 px-8 py-6 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center justify-between">
          ...
          <a href="{{ route('coupons.index') }}" class="group inline-flex items-center px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-2xl shadow hover:shadow-xl transition" style="font-size: 1.3rem;"><i class="ti ti-arrow-right mr-2"></i>عودة</a>
        </div>
        <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-16 translate-x-16"></div>
        <div class="absolute bottom-0 left-0 w-24 h-24 bg-white/10 rounded-full translate-y-12 -translate-x-12"></div>
      </div>
      <div class="p-8">
        <form method="POST" action="{{ route('coupons.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{mode:'selected'}">
          @csrf
          <div>
