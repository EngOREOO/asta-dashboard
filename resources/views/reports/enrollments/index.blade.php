      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-blue-600">{{ number_format($totalEnrollments) }}</p>
            @if($growthRate > 0)
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-lg font-bold text-purple-600">{{ $topCourses->first()->title ?? 'غير متاح' }}</p>
            <p class="text-sm text-gray-500 mt-1">{{ $topCourses->first()->students_count ?? 0 }} طالب</p>
          </div>
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-green-600">{{ $totalEnrollments > 0 ? round($totalEnrollments / max($topCourses->count(), 1), 1) : 0 }}</p>
...
new Chart(enrollmentsCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'التسجيلات',
            data: @json($chartData['enrollments']),
...
new Chart(categoryCtx, {
    type: 'doughnut',
    data: {
        labels: @json($enrollmentsByCategory->pluck('category_name')),
        datasets: [{
            data: @json($enrollmentsByCategory->pluck('enrollment_count')),
            backgroundColor: [
                'rgba(59, 130, 246, 0.8)',
                'rgba(168, 85, 247, 0.8)',
                'rgba(34, 197, 94, 0.8)',
...
function exportReport() {
    window.location.href = '{{ route("admin.reports.enrollments") }}?export=excel&start_date={{ $startDate->format("Y-m-d") }}&end_date={{ $endDate->format("Y-m-d") }}';
}
</script>
