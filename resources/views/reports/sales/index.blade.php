        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          ...
          <div>
            ...
            <select name="price_range" class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-300">
              <option value="all" {{ $priceRange == 'all' ? 'selected' : '' }}>جميع الدورات</option>
              <option value="paid" {{ $priceRange == 'paid' ? 'selected' : '' }}>مدفوعة فقط</option>
              <option value="free" {{ $priceRange == 'free' ? 'selected' : '' }}>مجانية فقط</option>
            </select>
          </div>
          ...
          <div class="flex items-end">
            <a href="{{ route('admin.reports.sales') }}" class="w-full bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded-xl text-center shadow-lg hover:shadow-xl transition-all duration-300">
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-green-600">{{ number_format($totalSales, 2) }} ريال</p>
            @if($growthRate > 0)
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ $paidCourses }} مدفوعة</p>
          </div>
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-purple-600">{{ $paidCourses }}</p>
            <p class="text-sm text-gray-500 mt-1">{{ $totalCourses > 0 ? round(($paidCourses / $totalCourses) * 100, 1) : 0 }}% من الإجمالي</p>
          </div>
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 p-6 hover:scale-105 transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-orange-600">{{ $freeCourses }}</p>
            <p class="text-sm text-gray-500 mt-1">{{ $totalCourses > 0 ? round(($freeCourses / $totalCourses) * 100, 1) : 0 }}% من الإجمالي</p>
          </div>
      ...
      <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20">
        ...
        <div class="p-8">
          <div class="h-80 relative">
            ...
            @if(empty($chartData['labels']) || empty($chartData['sales']))
              ...
              @foreach($topCourses as $index => $course)
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-gray-900 font-medium">
                    {{ number_format($course->price * $course->students_count, 2) }} ريال
                  </td>
                </tr>
              @endforeach
            </tbody>
...
function exportReport() {
    ...
    window.location.href = '{{ route("admin.reports.sales") }}?' + params.toString();
}
