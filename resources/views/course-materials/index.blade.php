        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @foreach($materials as $material)
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;">{{ $material->id }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="mr-4">
                    <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ $material->title }}</div>
                    @if($material->description)
                      <div class="text-gray-500" style="font-size: 1.3rem;">{{ Str::limit($material->description, 50) }}</div>
                    @endif
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ optional($material->course)->title ?? 'لا توجد دورة' }}</div>
                  @if(optional(optional($material->course)->instructor)->name)
                    <div class="text-gray-500" style="font-size: 1.3rem;">بواسطة {{ optional(optional($material->course)->instructor)->name }}</div>
                  @endif
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800" style="font-size: 1.3rem;">
                  {{ ucfirst($material->type ?? 'غير محدد') }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="{{ route('course-materials.destroy', $material) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه المادة؟')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
