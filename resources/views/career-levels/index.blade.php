  ...
  <div class="space-y-8 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="mt-4 sm:mt-0">
        <a href="{{ route('career-levels.create') }}" 
      ...
      <div class="p-6">
        <form method="GET" class="mb-6 grid grid-cols-1 md:grid-cols-5 gap-4">
          <div class="md:col-span-3">
            ...
            <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" placeholder="ابحث عن مستوى..." style="font-size: 1.3rem;">
          </div>
          <div>
          ...
          <div class="flex items-end gap-3">
            ...
            <a href="{{ route('career-levels.index') }}" class="career-action-btn career-action-btn--secondary px-5 py-2.5 rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200">مسح</a>
          </div>
        </form>
        ...
        <div class="admin-table-container">
          <table class="admin-table career-table-lg">
            ...
            <tbody>
              ...
              <tr>
                <td class="text-center">{{ ($levels->currentPage()-1)*$levels->perPage() + $loop->iteration }}</td>
                <td>{{ $level->name }}</td>
                <td class="text-center">
                  <span class="inline-flex px-2.5 py-0.5 rounded-full font-medium {{ $level->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">{{ $level->is_active ? 'نشط' : 'غير نشط' }}</span>
                </td>
                <td class="text-center">
                  <div class="inline-flex items-center gap-2">
                    <a href="{{ route('career-levels.edit', $level) }}" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-gray-700 bg-gray-100 hover:bg-gray-200" title="تعديل"><i class="ti ti-edit text-base"></i></a>
                    <form action="{{ route('career-levels.destroy', $level) }}" method="POST" class="inline" onsubmit="return confirm('حذف هذا المستوى؟')">
                      @csrf
                      @method('DELETE')
        ...
        @if($levels->hasPages())
          <div class="mt-6 flex justify-center">{{ $levels->links() }}</div>
        @endif
      </div>
    </div>
  </div>
