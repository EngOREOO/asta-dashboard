      ...
      <div class="p-8">
        <form action="{{ route('assessments.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input id="total_questions" name="total_questions" type="number" value="{{ old('total_questions') }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" style="font-size: 1.3rem;" min="1">
            <small class="text-gray-500">عدد الأسئلة المتوقع في التقييم</small>
            @error('total_questions')<p class="text-red-600 mt-1" style="font-size: 1.3rem;">{{ $message }}</p>@enderror
          </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div class="md:col-span-2">
                <select id="user_ids" name="user_ids[]" multiple class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" style="font-size: 1.3rem; min-height: 48px;">
                  ...
                  <option value="{{ $student->id }}" {{ collect(old('user_ids', []))->contains($student->id) ? 'selected' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function validateCurrentAndAddNew() {
    ...
    if (last) {
      ...
      try {
        const res = await fetch("{{ route('assessments.validate-question') }}", {
          method: 'POST',
          headers: {
  ...
  async function refreshSavedList() {
    try {
      const res = await fetch("{{ route('assessments.temp-questions') }}", { credentials: 'same-origin' });
      ...
      savedList.querySelectorAll('.save-saved').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          try {
            const res = await fetch(`{{ url('/assessments/temp-questions') }}/${i}`, {
              method: 'PUT',
