        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @foreach($assessments as $assessment)
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;">{{ $assessment->id }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="mr-4">
                    <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ $assessment->title }}</div>
                    @if($assessment->description)
                      <div class="text-gray-500" style="font-size: 1.3rem;">{{ Str::limit($assessment->description, 50) }}</div>
                    @endif
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ $assessment->course->title ?? 'لا توجد دورة' }}</div>
                  @if(optional($assessment->course->instructor)->name)
                    <div class="text-gray-500" style="font-size: 1.3rem;">بواسطة {{ $assessment->course->instructor->name }}</div>
                  @endif
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-800" style="font-size: 1.3rem;">
                  {{ $assessment->questions->count() ?? 0 }} سؤال
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-green-100 text-green-800" style="font-size: 1.3rem;">
                  {{ $assessment->attempts->count() ?? 0 }} محاولة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-gray-900" style="font-size: 1.3rem;">
                @if($assessment->duration_minutes)
                  <span class="flex items-center">
                    ...
                    {{ $assessment->duration_minutes }} دقيقة
                  </span>
                @else
              ...
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="{{ route('assessments.destroy', $assessment) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الاختبار؟')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
