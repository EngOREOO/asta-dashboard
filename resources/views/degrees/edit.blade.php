...
<div class="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-blue-100">
  <div class="space-y-8 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      <div class="space-y-2">
        ...
        <p class="text-lg text-gray-600 flex items-center">
          ...
          تحديث معلومات المسار المهنية: <span class="font-semibold text-gray-800">{{ $degree->name }}</span>
        </p>
      </div>
    ...
    <form method="POST" action="{{ route('degrees.update', $degree) }}" class="p-8">
      @csrf
      @method('PUT')
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="group space-y-3">
              ...
              <div class="relative group">
                ...
                <select id="level" 
                        name="level" 
                        class="block w-full pr-12 pl-4 py-4 bg-white/50 backdrop-blur-sm border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-green-500/20 focus:border-green-500 transition-all duration-300 hover:border-gray-300 hover:bg-white/70 @error('level') border-red-400 focus:ring-red-500/20 focus:border-red-500 @enderror"
                        required>
                  <option value="">اختر المستوى</option>
                  <option value="certificate" {{ old('level', $currentLevelString) == 'certificate' ? 'selected' : '' }}>شهادة</option>
                  <option value="diploma" {{ old('level', $currentLevelString) == 'diploma' ? 'selected' : '' }}>دبلوم</option>
                  <option value="bachelor" {{ old('level', $currentLevelString) == 'bachelor' ? 'selected' : '' }}>بكالوريوس</option>
                  <option value="master" {{ old('level', $currentLevelString) == 'master' ? 'selected' : '' }}>ماجستير</option>
                  <option value="doctorate" {{ old('level', $currentLevelString) == 'doctorate' ? 'selected' : '' }}>دكتوراه</option>
                </select>
              </div>
              @error('level')
              <div class="flex items-center space-x-2 space-x-reverse text-red-600 bg-red-50 px-4 py-3 rounded-xl border border-red-200 animate-shake shadow-sm">
            ...
            <div class="group space-y-3">
              ...
              <div class="relative group">
                ...
                <input type="number" 
                       ...
                       value="{{ old('duration_months', $degree->duration_months) }}"
                       min="1"
                       class="block w-full pr-12 pl-4 py-4 bg-white/50 backdrop-blur-sm border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 transition-all duration-300 hover:border-gray-300 hover:bg-white/70 @error('duration_months') border-red-400 focus:ring-red-500/20 focus:border-red-500 @enderror"
            ...
            <div class="group space-y-3">
              ...
              <div class="relative group">
                ...
                <input type="number"
                       id="credit_hours"
                       name="credit_hours"
                       value="{{ old('credit_hours', $degree->credit_hours) }}"
                       min="0"
                       class="block w-full pr-12 pl-4 py-4 bg-white/50 backdrop-blur-sm border-2 border-gray-200 rounded-2xl focus:ring-4 focus:ring-amber-500/20 focus:border-amber-500 transition-all duration-300 hover:border-gray-300 hover:bg-white/70 @error('credit_hours') border-red-400 focus:ring-red-500/20 focus:border-red-500 @enderror"
          ...
          <div class="group space-y-3">
            ...
            <div class="relative group">
              ...
              <textarea id="description" 
                        ...
                        placeholder="أدخل وصف المسار المهنية">{{ old('description', $degree->description) }}</textarea>
            </div>
            @error('description')
            <div class="flex items-center space-x-2 space-x-reverse text-red-600 bg-red-50 px-4 py-3 rounded-xl border border-red-200 animate-shake shadow-sm">
              <i class="ti ti-alert-circle text-lg"></i>
          ...
          <div class="group space-y-3">
            ...
            <div class="flex items-center space-x-4 space-x-reverse bg-white/50 backdrop-blur-sm rounded-2xl p-4 border-2 border-gray-200 hover:border-gray-300 transition-all duration-300">
              ...
              <div class="flex flex-col">
                <span class="text-sm font-medium {{ old('is_active', $degree->is_active) ? 'text-green-700' : 'text-red-700' }}" id="status-text">
                  {{ old('is_active', $degree->is_active) ? 'نشط' : 'غير نشط' }}
                </span>
                <span class="text-xs {{ old('is_active', $degree->is_active) ? 'text-green-600' : 'text-red-600' }}">
                  {{ old('is_active', $degree->is_active) ? 'المسار مرئية للجمهور' : 'المسار مخفية' }}
                </span>
              </div>
            </div>
          </div>
              ...
              <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $degree->courses_count ?? 0 }}</p>
                  </div>
              ...
              <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{ ucfirst($currentLevelString ?? 'certificate') }}</p>
                  </div>
              ...
              <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{ $degree->duration_months }} شهر</p>
                  </div>
              ...
              <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{ $degree->created_at->format('M Y') }}</p>
                  </div>
