        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @foreach($instructors as $instructor)
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900 text-center" style="font-size: 1.3rem;">{{ $instructor->id }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-center">
                @if($instructor->profile_photo_path && Storage::disk('public')->exists($instructor->profile_photo_path))
              ...
              <td class="px-6 py-4 whitespace-nowrap text-rtl">
                <div>
                  <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ $instructor->name }}</div>
                  @if($instructor->bio)
                  <div class="text-gray-500" style="font-size: 1.3rem;">{{ Str::limit($instructor->bio, 50) }}</div>
                  @endif
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-ltr">
                <div class="text-gray-900 email-content" style="font-size: 1.3rem;">{{ $instructor->email }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-rtl">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800" style="font-size: 1.3rem;">
                  {{ $instructor->courses_count ?? 0 }} دورة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-rtl">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-rtl text-gray-900" style="font-size: 1.3rem;">
                {{ $instructor->created_at->format('Y-n-j') }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-center">
                <form action="{{ route('users.toggle-active', $instructor) }}" method="POST">
                  @csrf
                  <button type="submit" class="inline-flex items-center px-3 py-1.5 rounded-full text-sm {{ $instructor->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700' }}">
                    <i class="ti {{ $instructor->is_active ? 'ti-toggle-right' : 'ti-toggle-left' }} ml-1"></i>
              ...
              <td class="px-6 py-4 whitespace-nowrap font-medium text-center" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="{{ route('users.destroy', $instructor) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا المحاضر؟')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
