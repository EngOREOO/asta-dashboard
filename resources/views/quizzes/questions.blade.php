                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                  <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                      <div class="flex items-center">
                        <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl flex items-center justify-center mr-4">
                          <span class="text-white font-bold text-sm">Q{{ $question->order }}</span>
                        </div>
                        <div class="flex-grow">
                          <h3 class="font-semibold text-gray-900 mb-2" style="font-size: 1.3rem;">{{ Str::limit($question->question, 80) }}</h3>
                      ...
                      <div class="flex items-center space-x-2 space-x-reverse">
                        <a href="{{ route('quizzes.questions.edit', [$quiz, $question]) }}" class="group inline-flex items-center p-2 bg-gray-50 text-gray-600 rounded-xl hover:bg-gray-100 hover:scale-110 transition-all duration-200" title="تعديل">
                          <i class="ti ti-edit group-hover:scale-110 transition-transform duration-200"></i>
                        </a>
                        <form action="{{ route('quizzes.questions.destroy', [$quiz, $question]) }}" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا السؤال؟')">
                          @csrf
                          @method('DELETE')
                    ...
                    <div class="border-t border-gray-200 pt-4">
                      <button class="flex items-center justify-between w-full text-right text-gray-600 hover:text-gray-900 transition-colors duration-200" onclick="toggleQuestionDetails({{ $question->id }})">
                        <span class="font-medium" style="font-size: 1.2rem;">عرض التفاصيل</span>
                        <i class="ti ti-chevron-down transition-transform duration-200" id="chevron-{{ $question->id }}"></i>
                      </button>
                      
                      <div id="details-{{ $question->id }}" class="mt-4 hidden">
                        <div class="bg-gray-50 p-4 rounded-xl">
                          <div class="mb-4">
                            ...
                            <p class="text-gray-700 leading-relaxed" style="font-size: 1.3rem;">{{ $question->question }}</p>
                            
                            @if($question->image)
                            <div class="mt-3">
                              <img src="{{ asset('storage/' . $question->image) }}" alt="صورة السؤال" class="rounded-xl shadow-sm" style="max-height: 200px;">
                            </div>
                            @endif
                          </div>
                          ...
                          <div class="mb-4">
                            ...
                            @if($question->type === 'multiple_choice')
                              <div class="space-y-2">
                                ...
                                <div class="flex items-center p-3 bg-white rounded-xl border {{ $answer->is_correct ? 'border-green-200 bg-green-50' : 'border-gray-200' }}">
                                  ...
                                  <div class="flex-grow">
                                    <p class="text-gray-700 font-medium" style="font-size: 1.2rem;">{{ $answer->answer_text }}</p>
                                    @if($answer->feedback)
                                      <p class="text-gray-500 text-sm mt-1">{{ $answer->feedback }}</p>
                                    @endif
                                  </div>
                                  @if($answer->is_correct)
                              ...
                              <div class="p-4 bg-white rounded-xl border border-gray-200">
                                <div class="flex items-center">
                                  ...
                                  <span class="inline-flex items-center px-3 py-1 bg-green-100 text-green-800 rounded-lg font-semibold" style="font-size: 1.2rem;">
                                    {{ $question->correctAnswers->first()->answer_text === 'true' ? 'صح' : 'خطأ' }}
                                  </span>
                                </div>
                              </div>
                            @else
                              <div class="p-4 bg-white rounded-xl border border-gray-200">
                                <p class="text-gray-700 font-medium" style="font-size: 1.2rem;">{{ $question->correctAnswers->first()->answer_text ?? 'لم يتم توفير إجابة' }}</p>
                              </div>
                            @endif
                          </div>
                          ...
                          <div class="mb-4">
                            ...
                            <div class="p-4 bg-purple-50 border border-purple-200 rounded-xl">
                              <p class="text-gray-700 leading-relaxed" style="font-size: 1.2rem;">{{ $question->explanation }}</p>
                            </div>
                          </div>
                          @endif
                        </div>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6 mb-6">
              ...
              <div class="text-center">
                ...
                <h4 class="text-2xl font-bold text-gray-900 mb-1">{{ $quiz->questions->sum('points') }}</h4>
