  ...
  <div class="space-y-8 p-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="mt-4 sm:mt-0">
        <a href="{{ route('quizzes.index') }}" class="inline-flex items-center px-6 py-3 bg-white text-gray-700 border border-gray-200 rounded-2xl hover:bg-gray-50 shadow-sm" style="font-size: 1.3rem;">
    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 animate-slide-up">
      ...
      <div class="p-8">
        <form action="{{ route('quizzes.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          @csrf
          ...
          <div>
            ...
            <input id="available_from" name="available_from" type="datetime-local" value="{{ old('available_from') }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" style="font-size: 1.3rem;">
            @error('available_from')<p class="text-red-600 mt-1" style="font-size: 1.3rem;">{{ $message }}</p>@enderror
          </div>

          <div>
            <label for="available_until" class="block text-gray-700 mb-2" style="font-size: 1.3rem;">متاح حتى</label>
            <input id="available_until" name="available_until" type="datetime-local" value="{{ old('available_until') }}" class="w-full border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500" style="font-size: 1.3rem;">
            @error('available_until')<p class="text-red-600 mt-1" style="font-size: 1.3rem;">{{ $message }}</p>@enderror
          </div>
          ...
          <div class="md:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-4 items-center">
            ...
            <label class="inline-flex items-center gap-2">
              <input class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" type="checkbox" id="show_results_immediately" name="show_results_immediately" value="1" {{ old('show_results_immediately', true) ? 'checked' : '' }}>
              <span style="font-size: 1.3rem;">عرض النتائج فوراً</span>
            </label>
            <label class="inline-flex items-center gap-2">
              <input class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" type="checkbox" id="allow_review" name="allow_review" value="1" {{ old('allow_review', true) ? 'checked' : '' }}>
          ...
          <div class="md:col-span-2 flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('quizzes.index') }}" class="inline-flex items-center px-6 py-3 bg-white text-gray-700 border border-gray-200 rounded-2xl hover:bg-gray-50 shadow-sm" style="font-size: 1.3rem;">إلغاء</a>
          </div>
        </form>
      </div>
    </div>
