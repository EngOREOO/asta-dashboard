  ...
  <div class="space-y-8 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="mt-4 sm:mt-0">
        <a href="{{ route('student-progress.analytics') }}" 
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          @foreach($students as $student)
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;">{{ $student->id }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="mr-4">
                    <div class="font-medium text-gray-900" style="font-size: 1.3rem;">{{ $student->name }}</div>
                    <div class="text-gray-500 email-content" style="font-size: 1.3rem;">{{ $student->email }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800" style="font-size: 1.3rem;">
                  {{ $student->enrolled_courses_count }} دورة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-green-100 text-green-800" style="font-size: 1.3rem;">
                  {{ $student->completed_courses }} مكتملة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-yellow-100 text-yellow-800" style="font-size: 1.3rem;">
                  {{ $student->in_progress_courses }} قيد التقدم
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div class="flex-1 bg-gray-200 rounded-full h-2 mr-3">
                    <div class="bg-gradient-to-r from-teal-500 to-blue-500 h-2 rounded-full transition-all duration-300" style="width: {{ $student->total_progress }}%"></div>
                  </div>
                  <span class="text-sm font-medium text-gray-900" style="font-size: 1.3rem;">{{ $student->total_progress }}%</span>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-purple-100 text-purple-800" style="font-size: 1.3rem;">
                  {{ $student->material_completions_count }} مادة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  <a href="{{ route('student-progress.show', $student) }}" 
