  ...
  <div class="space-y-8 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="mt-4 sm:mt-0">
        <a href="<?php echo e(route('degrees.create')); ?>" 
      ...
      <div class="p-8">
        ...
        <form method="GET" class="mb-6 grid grid-cols-1 md:grid-cols-6 gap-4">
          <div class="md:col-span-2">
            ...
            <input type="text" name="q" value="<?php echo e($filters['q'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" placeholder="ابحث في المسارات..." />
          </div>
          <div>
            ...
            <select name="level" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500">
              ...
              <?php $__currentLoopData = $levels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k=>$v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($k); ?>" <?php if(($filters['level'] ?? '')===$k): echo 'selected'; endif; ?>><?php echo e($v); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">مدة من (شهر)</label>
            <input type="number" name="duration_min" value="<?php echo e($filters['duration_min'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">مدة إلى (شهر)</label>
            <input type="number" name="duration_max" value="<?php echo e($filters['duration_max'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">الساعات من</label>
            <input type="number" name="credit_min" value="<?php echo e($filters['credit_min'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">الساعات إلى</label>
            <input type="number" name="credit_max" value="<?php echo e($filters['credit_max'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <select name="courses_order" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500">
              ...
              <option value="desc" <?php if(($filters['courses_order'] ?? '')==='desc'): echo 'selected'; endif; ?>>من الأعلى للأقل</option>
              <option value="asc" <?php if(($filters['courses_order'] ?? '')==='asc'): echo 'selected'; endif; ?>>من الأقل للأعلى</option>
            </select>
          </div>
          <div class="md:col-span-6 flex items-end gap-3">
            ...
            <a href="<?php echo e(route('degrees.index')); ?>" class="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200">مسح</a>
          </div>
        </form>
        <?php if(isset($degrees) && $degrees->count() > 0): ?>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <?php $__currentLoopData = $degrees; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $degree): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($degree->id); ?></td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  ...
                  <div class="mr-4">
                    <div class="font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($degree->name); ?></div>
                    <?php if($degree->description): ?>
                      <div class="text-gray-500" style="font-size: 1.3rem;"><?php echo e(Str::limit($degree->description, 50)); ?></div>
                    <?php endif; ?>
                  </div>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800" style="font-size: 1.3rem;">
                  <?php echo e(ucfirst($degree->level ?? 'certificate')); ?>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-800" style="font-size: 1.3rem;">
                  <?php echo e($degree->courses_count ?? 0); ?> دورة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="<?php echo e(route('degrees.destroy', $degree)); ?>" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه المسار المهنية؟')">
                    <?php echo csrf_field(); ?>
        ...
        <!-- Pagination -->
        <?php if($degrees->hasPages()): ?>
          <div class="mt-6 flex justify-center">
            <?php echo e($degrees->links()); ?>
...
<?php echo $__env->make('layouts.dash', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ahmed\Documents\asta\asss\resources\views/degrees/index.blade.php ENDPATH**/ ?>