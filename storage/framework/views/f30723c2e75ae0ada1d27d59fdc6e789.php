    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 animate-slide-up">
      ...
      <div class="p-8">
        ...
        <form method="GET" class="mb-6 grid grid-cols-1 md:grid-cols-6 gap-4">
          ...
          <?php if(isset($instructors) && $instructors->count()): ?>
          <div>
            ...
            <select name="instructor_id" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500">
              ...
              <?php $__currentLoopData = $instructors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $inst): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($inst->id); ?>" <?php if(($filters['instructor_id'] ?? '') == $inst->id): echo 'selected'; endif; ?>><?php echo e($inst->name); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
          </div>
          <?php endif; ?>
          <div>
            ...
            <input type="number" step="0.01" name="price_min" value="<?php echo e($filters['price_min'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">السعر الأقصى</label>
            <input type="number" step="0.01" name="price_max" value="<?php echo e($filters['price_max'] ?? ''); ?>" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500" />
          </div>
          <div>
            ...
            <select name="discount" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500">
              <option value="">الكل</option>
              <option value="yes" <?php if(($filters['discount'] ?? '')==='yes'): echo 'selected'; endif; ?>>به خصم</option>
              <option value="no" <?php if(($filters['discount'] ?? '')==='no'): echo 'selected'; endif; ?>>بدون خصم</option>
            </select>
          </div>
          <div>
          ...
          <div>
            ...
            <select name="students_order" class="w-full rounded-xl border-gray-300 focus:border-cyan-500 focus:ring-cyan-500">
              ...
              <option value="desc" <?php if(($filters['students_order'] ?? '')==='desc'): echo 'selected'; endif; ?>>من الأعلى إلى الأقل</option>
              <option value="asc" <?php if(($filters['students_order'] ?? '')==='asc'): echo 'selected'; endif; ?>>من الأقل إلى الأعلى</option>
            </select>
          </div>
          <div class="md:col-span-6 flex items-end gap-3">
            ...
            <a href="<?php echo e(route('courses.index')); ?>" class="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200">مسح</a>
          </div>
        </form>
        ...
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="px-6 py-4 text-right font-medium text-black uppercase tracking-wider" style="font-size: 1.3rem;">كود الدورة</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <?php $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($course->id); ?></td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <div class="font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($course->title); ?></div>
                  <?php if($course->description): ?>
                  <div class="text-gray-500" style="font-size: 1.3rem;"><?php echo e(Str::limit($course->description, 50)); ?></div>
                  <?php endif; ?>
                </div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-gray-900" style="font-size: 1.3rem;">
                  <?php echo e(optional($course->instructor)->name ?? 'بدون مدرس'); ?>

                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-gray-900" style="font-size: 1.3rem;">
                  <?php echo e(optional($course->category)->name ?? '—'); ?>
                ...
                <?php if($original > 0): ?>
                  <?php if($discounted < $original): ?>
                    <div class="flex items-center gap-2">
                      <span class="line-through text-gray-400" style="font-size: 1.2rem;"><?php echo e(number_format($original, 2)); ?></span>
                      <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-emerald-100 text-emerald-800" style="font-size: 1.3rem;">
                        ...
                        <img src="<?php echo e(asset('riyal.svg')); ?>" style="width: 1.5rem; height: 1.5rem; filter: brightness(0);" alt="ريال" class="w-3 h-3 ml-1">
                      </span>
                    </div>
                  <?php else: ?>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-900" style="font-size: 1.3rem;">
                <?php ($original = (float)($course->price ?? 0)); ?>
                <?php ($discounted = (float)$course->discounted_price); ?>
                <?php if($original > 0 && $discounted < $original): ?>
                  <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-pink-100 text-pink-800" style="font-size: 1.3rem;">
                    خصم <?php echo e(round((1 - ($discounted/$original)) * 100)); ?>%
                  </span>
                <?php else: ?>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-900" style="font-size: 1.3rem;">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-800" style="font-size: 1.3rem;">
                  <?php echo e($course->students_count ?? 0); ?> طالب
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="<?php echo e(route('courses.destroy', $course)); ?>" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه الدورة؟')">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.dash', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ahmed\Documents\asta\asss\resources\views/courses/index.blade.php ENDPATH**/ ?>