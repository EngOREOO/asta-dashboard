...
x-init="$nextTick(()=>{ 
  <?php if(session('success')): ?>
    toastMessage='<?php echo e(addslashes(session('success'))); ?>'; toastType='success'; toastShow=true; setTimeout(()=>toastShow=false,3000);
  <?php endif; ?>
  <?php if(session('error')): ?>
    toastMessage='<?php echo e(addslashes(session('error'))); ?>'; toastType='error'; toastShow=true; setTimeout(()=>toastShow=false,3000);
  <?php endif; ?>
})">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="mt-4 sm:mt-0">
        <span class="inline-flex items-center px-4 py-2 rounded-xl bg-amber-100 text-amber-800 border border-amber-200" style="font-size: 1.1rem;">
          <?php echo e($applications->where('status', 'pending')->count()); ?> طلب قيد المراجعة
        </span>
      </div>
    </div>
          ...
          <table class="admin-table" id="applications-table">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr class="hover:bg-gray-50 transition-colors duration-200">
                <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900 text-center" style="font-size: 1.3rem;"><?php echo e($application->id); ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-rtl">
                  <div class="flex items-center space-x-3 space-x-reverse">
                    <div class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center text-white font-semibold">
                      <?php echo e(Str::of($application->user->name ?? 'U')->split('/\s+/')->map(fn($s)=>Str::substr($s,0,1))->implode('')); ?>

                    </div>
                    <div>
                      <div class="font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e(optional($application->user)->name ?? '—'); ?></div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-ltr">
                  <div class="text-gray-900 email-content" style="font-size: 1.3rem;"><?php echo e(optional($application->user)->email ?? '—'); ?></div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap align-middle">
                  ...
                  ?>
                  <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium <?php echo e($badge); ?>" style="font-size: 1.2rem;"><?php echo e($statusAr); ?></span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-rtl text-gray-900" style="font-size: 1.3rem;"><?php echo e($application->created_at ? $application->created_at->format('Y-n-j') : '—'); ?></td>
                <td class="px-6 py-4 whitespace-nowrap text-rtl">
                  <?php if($application->reviewer): ?>
                    <div class="text-gray-900" style="font-size: 1.3rem;"><?php echo e($application->reviewer->name); ?></div>
                    <div class="text-gray-500" style="font-size: 1.1rem;"><?php echo e($application->reviewed_at ? $application->reviewed_at->format('Y-n-j') : ''); ?></div>
                  <?php else: ?>
                ...
                <td class="px-6 py-4 whitespace-nowrap font-medium text-center" style="font-size: 1.3rem;">
                  <div class="inline-flex items-center gap-2">
                    <a href="<?php echo e(route('instructor-applications.show', $application)); ?>" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-blue-700 bg-blue-100 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" title="عرض">
                      <i class="ti ti-eye text-base"></i>
                    </a>
                    <?php if($application->status === 'pending'): ?>
                      <!-- <button type="button" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-green-700 bg-green-100 hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="window.openApprove(<?php echo e($application->id); ?>, '<?php echo e(optional($application->user)->name ?? '—'); ?>')" title="قبول">
                        <i class="ti ti-check text-base"></i>
                      </button> -->
                      <!-- <button type="button" class="h-9 w-9 inline-flex items-center justify-center rounded-lg text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500" onclick="window.openReject(<?php echo e($application->id); ?>, '<?php echo e(optional($application->user)->name ?? '—'); ?>')" title="رفض">
...
<?php echo $__env->make('layouts.dash', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ahmed\Documents\asta\asss\resources\views/instructor-applications/index.blade.php ENDPATH**/ ?>