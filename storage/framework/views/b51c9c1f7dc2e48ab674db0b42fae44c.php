    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      <div class="space-y-2">
        ...
        <p class="text-gray-600 flex items-center" style="font-size: 1.3rem;">
          <i class="ti ti-clipboard-list mr-2 text-cyan-500"></i>
          إدارة الاختبارات والامتحانات في النظام
        </p>
      </div>
      <div class="mt-4 sm:mt-0 flex gap-3">
        <a href="<?php echo e(route('assessments.analytics')); ?>" 
    ...
    <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-white/20 animate-slide-up">
      <div class="bg-gradient-to-r from-teal-500 via-cyan-500 to-blue-600 px-8 py-6 relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <p class="text-blue-100 mt-2" style="font-size: 1.3rem;">عرض وإدارة جميع الاختبارات والامتحانات</p>
        </div>
        <!-- Decorative elements -->
      ...
      <div class="p-8">
        <?php if(isset($assessments) && $assessments->count() > 0): ?>
        <div class="table-responsive">
          <table class="table table-hover" id="assessments-table">
        <thead class="bg-gray-50">
          <tr>
            <th class="px-6 py-4 text-right font-medium text-black uppercase tracking-wider" style="font-size: 1.3rem;">#</th>
            <th class="px-6 py-4 text-right font-medium text-black uppercase tracking-wider" style="font-size: 1.3rem;">الاختبار</th>
            ...
            <th class="px-6 py-4 text-right font-medium text-black uppercase tracking-wider" style="font-size: 1.3rem;">الأسئلة</th>
            <th class="px-6 py-4 text-right font-medium text-black uppercase tracking-wider" style="font-size: 1.3rem;">المحاولات</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <?php $__currentLoopData = $assessments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $assessment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="hover:bg-gray-50 transition-colors duration-200">
              <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($assessment->id); ?></td>
                ...
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <div class="h-10 w-10 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center shadow-sm">
                      <i class="ti ti-clipboard-list text-white text-lg"></i>
                    </div>
                  </div>
                  <div class="mr-4">
                    <div class="font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($assessment->title); ?></div>
                    <?php if($assessment->description): ?>
                      <div class="text-gray-500" style="font-size: 1.3rem;"><?php echo e(Str::limit($assessment->description, 50)); ?></div>
                    <?php endif; ?>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <div>
                  <div class="font-medium text-gray-900" style="font-size: 1.3rem;"><?php echo e($assessment->course->title ?? 'لا توجد دورة'); ?></div>
                  <?php if(optional($assessment->course->instructor)->name): ?>
                    <div class="text-gray-500" style="font-size: 1.3rem;">بواسطة <?php echo e($assessment->course->instructor->name); ?></div>
                  <?php endif; ?>
                </div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-800" style="font-size: 1.3rem;">
                  <?php echo e(ucfirst($assessment->type ?? 'quiz')); ?>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-indigo-100 text-indigo-800" style="font-size: 1.3rem;">
                  <?php echo e($assessment->questions->count() ?? 0); ?> سؤال
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full font-medium bg-green-100 text-green-800" style="font-size: 1.3rem;">
                  <?php echo e($assessment->attempts->count() ?? 0); ?> محاولة
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-gray-900" style="font-size: 1.3rem;">
                <?php if($assessment->duration_minutes): ?>
                  <span class="flex items-center">
                    ...
                    <?php echo e($assessment->duration_minutes); ?> دقيقة
                  </span>
                <?php else: ?>
                  <span class="text-gray-400">بدون حد</span>
                <?php endif; ?>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap font-medium" style="font-size: 1.3rem;">
                <div class="flex items-center space-x-2 space-x-reverse">
                  ...
                  <form action="<?php echo e(route('assessments.destroy', $assessment)); ?>" method="POST" class="inline" onsubmit="return confirm('هل أنت متأكد من حذف هذا الاختبار؟')">
                    <?php echo csrf_field(); ?>
        ...
        <div class="text-center py-12">
          <div class="mb-6">
            <div class="w-24 h-24 bg-gradient-to-r from-blue-100 to-purple-100 rounded-full flex items-center justify-center mx-auto">
              <i class="ti ti-clipboard-list text-4xl text-blue-500"></i>
            </div>
          </div>
          <h3 class="font-medium text-gray-900 mb-2" style="font-size: 1.9rem;">لا توجد اختبارات</h3>
          <p class="text-gray-500 mb-6" style="font-size: 1.3rem;">ابدأ بإنشاء اختبارات لتقييم تعلم الطلاب.</p>
          <a href="<?php echo e(route('assessments.create')); ?>" 
            ...
            <i class="ti ti-square-plus mr-2"></i>إنشاء أول اختبار
          </a>
        </div>
        <?php endif; ?>
      </div>
...
<?php echo $__env->make('layouts.dash', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ahmed\Documents\asta\asss\resources\views/assessments/index.blade.php ENDPATH**/ ?>