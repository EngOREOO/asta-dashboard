  ...
  <div class="space-y-8 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-fade-in">
      ...
      <div class="flex gap-3">
        ...
        <a href="<?php echo e($certificate->certificate_url); ?>" target="_blank" class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-2xl shadow hover:bg-indigo-700" style="font-size: 1.3rem;">
          <i class="ti ti-download mr-2"></i>تنزيل
        </a>
        <?php endif; ?>
        <a href="<?php echo e(route('certificates.index')); ?>" class="inline-flex items-center px-6 py-3 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-2xl text-gray-700 shadow hover:shadow-lg" style="font-size: 1.3rem;">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div id="cert-variant-1" class="relative rounded-[28px] overflow-hidden shadow-xl bg-white">
          <div class="certificate-svg-container">
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="100%" height="auto" viewBox="0 0 1684 1191" class="w-full h-auto">
              ...
              <!-- Student Name -->
              <text x="842" y="350" text-anchor="middle" font-family="Arial, sans-serif" font-size="36" font-weight="bold" fill="#2C3E50"><?php echo e(optional($certificate->user)->name ?? '—'); ?></text>
              <text x="842" y="380" text-anchor="middle" font-family="Arial, sans-serif" font-size="18" fill="#7F8C8D"><?php echo e(optional($certificate->user)->email ?? ''); ?></text>
              
              <!-- Course Information -->
              <text x="842" y="450" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" fill="#34495E">لإتمامه بنجاح دورة</text>
              <text x="842" y="490" text-anchor="middle" font-family="Arial, sans-serif" font-size="32" font-weight="bold" fill="#2C3E50"><?php echo e(optional($certificate->course)->title ?? '—'); ?></text>
              
              <!-- Course Details -->
              <text x="842" y="550" text-anchor="middle" font-family="Arial, sans-serif" font-size="20" fill="#7F8C8D">المحاضر: <?php echo e(optional($certificate->course->instructor)->name ?? '—'); ?></text>
              <text x="842" y="580" text-anchor="middle" font-family="Arial, sans-serif" font-size="20" fill="#7F8C8D">القسم: <?php echo e(optional($certificate->course->category)->name ?? '—'); ?></text>
              
              <!-- Issue Date -->
              <text x="842" y="650" text-anchor="middle" font-family="Arial, sans-serif" font-size="18" fill="#95A5A6">تاريخ الإصدار: <?php echo e($certificate->issued_at ? \Carbon\Carbon::parse($certificate->issued_at)->format('Y-m-d') : '—'); ?></text>
              
              <!-- Certificate ID -->
              <text x="842" y="680" text-anchor="middle" font-family="Arial, sans-serif" font-size="16" fill="#BDC3C7">رقم الشهادة: #<?php echo e($certificate->id); ?></text>
          ...
          <div class="mt-6 flex items-center justify-end gap-3">
            <?php if($certificate->certificate_url): ?>
            <a href="<?php echo e($certificate->certificate_url); ?>" target="_blank" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 text-white rounded-xl shadow hover:bg-indigo-700"><i class="ti ti-download mr-2"></i>تحميل الملف</a>
            <?php endif; ?>
          ...
          <div class="relative p-10 rounded-[24px]" style="background:#fff; border:4px solid transparent; border-image: linear-gradient(45deg, #06b6d4, #3b82f6, #1e40af) 1; box-shadow: inset 0 0 0 10px rgba(59,130,246,0.08);">
            ...
            <div class="flex items-center justify-between mb-8">
              <div class="flex items-center gap-4">
                <img src="<?php echo e(asset('images/asta-logo.png')); ?>" alt="ASTA" class="h-10 w-auto" />
                <div>
                  <div class="text-gray-900 font-extrabold tracking-widest" style="font-size: 1.3rem;">ASTA Academy</div>
                  <div class="text-gray-500" style="font-size: .95rem;">Certificate ID: #<?php echo e($certificate->id); ?></div>
                </div>
              </div>
              <div class="text-gray-600" style="font-size: .95rem;"><?php echo e(now()->format('Y-n-j')); ?></div>
            </div>
            ...
            <div class="text-center">
              ...
              <div class="mt-6">
                <div class="inline-block px-10 py-3 rounded-2xl" style="border:2px solid transparent; border-image: linear-gradient(90deg, #06b6d4, #3b82f6) 1;">
                  <div class="font-serif text-gray-900" style="font-size: 1.9rem;"><?php echo e(optional($certificate->user)->name ?? '—'); ?></div>
                </div>
                <div class="email-content text-ltr mt-1 text-gray-500"><?php echo e(optional($certificate->user)->email ?? ''); ?></div>
              </div>

              <p class="mt-6 text-gray-700 font-serif" style="font-size: 1.15rem;">has successfully completed the course</p>
              <div class="font-semibold text-gray-900" style="font-size: 1.5rem;"><?php echo e(optional($certificate->course)->title ?? '—'); ?></div>

              <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div>
                  <div class="text-gray-600 mb-1">Instructor</div>
                  <div class="font-medium"><?php echo e(optional($certificate->course->instructor)->name ?? '—'); ?></div>
                </div>
                <div>
                  <div class="text-gray-600 mb-1">Category</div>
                  <div class="font-medium"><?php echo e(optional($certificate->course->category)->name ?? '—'); ?></div>
                </div>
                <div>
                  <div class="text-gray-600 mb-1">Issued on</div>
                  <div class="font-medium"><?php echo e($certificate->issued_at ? \Carbon\Carbon::parse($certificate->issued_at)->format('Y-n-j') : '—'); ?></div>
                </div>
              </div>
      ...
      <div class="lg:col-span-1 space-y-6">
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-6">
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              ...
              <span class="font-medium"><?php echo e($certificate->id); ?></span>
            </div>
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              ...
              <span class="font-medium"><?php echo e($certificate->created_at ? $certificate->created_at->format('Y-n-j') : '—'); ?></span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-6">
          ...
          <div class="space-y-3">
            <?php if($certificate->certificate_url): ?>
            <a href="<?php echo e($certificate->certificate_url); ?>" target="_blank" class="w-full inline-flex items-center justify-center px-4 py-2 bg-indigo-600 text-white rounded-xl shadow hover:bg-indigo-700"><i class="ti ti-download mr-2"></i>تنزيل الشهادة</a>
            <?php endif; ?>
            <form action="<?php echo e(route('certificates.destroy', $certificate)); ?>" method="POST" onsubmit="return confirm('سحب الشهادة؟')">
              <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.dash', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ahmed\Documents\asta\asss\resources\views/certificates/show.blade.php ENDPATH**/ ?>